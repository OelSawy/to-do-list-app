...
from services.tasks.task_handling import create_task, edit_task, get_all_tasks, get_task, toggle_task_status, delete_task, delete_completed_tasks, complete_all_tasks
...
@router.put("/edit/{task_id}", response_model=TaskEditResponse)
...
@router.put("/toggle/{task_id}", response_model=TaskResponse)
...
def delete_completed(request: Request):
    try:
...
@router.put("/toggle")
def toggle_all(request: Request):
    try:
        complete_all_tasks(request)
        return Response(status_code=204)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))